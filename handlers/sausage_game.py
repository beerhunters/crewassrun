...
GAME_DISABLED_MESSAGE = "Сосискомёт на переучёте! Попробуй позже! 🌭"
...
async def sausage_throw_handler(message: Message, bot: Bot):
    ...
    game_enabled = await get_game_setting(
        "sausage_game_enabled"
    )  # По умолчанию включено (1)
    if game_enabled == 0:
        await message.reply(GAME_DISABLED_MESSAGE)
        return
    ...
    # Проверка очков перед броском
    SAUSAGE_THROW_COST = await get_game_setting("sausage_throw_cost")  # Установлено 2
    attacker_points = await get_user_points(attacker.id, chat_id)
    if attacker_points < SAUSAGE_THROW_COST:
        await handle_no_points(
            bot, chat_id, attacker, attacker_points, SAUSAGE_THROW_COST
