...
from database.repo import (
    ...
    update_game_setting,
    delete_game_setting,
    get_user_by_id,  # Добавлен для toggle_user_game_endpoint
)
from database.models import User, Bun, UserBun, GameSetting
...
@app.put("/users/{telegram_id}/{chat_id}/toggle_game")
async def toggle_user_game_endpoint(telegram_id: int, chat_id: int):
    async with async_session() as session:
        if not await toggle_user_game(session, telegram_id, chat_id):
            raise HTTPException(status_code=404, detail="Пользователь не найден")
...
# --- Эндпоинты для булочек ---
@app.get("/buns/", response_model=List[BunModel])
async def get_buns_endpoint():
    async with async_session() as session:
        buns = await get_buns(session)
        return [BunModel(**bun.__dict__) for bun in buns]


@app.post("/buns/", response_model=BunModel)
async def create_bun_endpoint(bun: BunModel):
    async with async_session() as session:
        new_bun = await add_bun(session, bun.name, bun.points)
        if not new_bun:
            raise HTTPException(status_code=400, detail="Bun already exists")
        return bun


@app.put("/buns/{name}", response_model=BunModel)
async def update_bun_endpoint(name: str, bun: BunModel):
    async with async_session() as session:
        updated_bun = await update_bun(session, name, bun.points)
        if not updated_bun:
            raise HTTPException(status_code=404, detail="Bun not found")
        return bun
...
@app.delete("/buns/{name}")
async def delete_bun_endpoint(name: str):
    async with async_session() as session:
        if not await delete_bun(session, name):
            raise HTTPException(status_code=404, detail="Bun not found")
        return {"message": f"Bun {name} deleted"}
...
# --- Эндпоинты для результатов (UserBun) ---
@app.get("/user_buns/", response_model=List[UserBunModel])
async def get_user_buns_endpoint():
    async with async_session() as session:
        user_buns = await get_user_buns(session)
        return [UserBunModel(**ub.__dict__) for ub in user_buns]


@app.put("/user_buns/{id}", response_model=UserBunModel)
async def update_user_bun_endpoint(id: int, user_bun: UserBunModel):
    async with async_session() as session:
        updated_ub = await update_user_bun(session, id, user_bun.count, user_bun.points)
        if not updated_ub:
            raise HTTPException(status_code=404, detail="UserBun not found")
        return user_bun
...
@app.delete("/user_buns/{id}")
async def delete_user_bun_endpoint(id: int):
    async with async_session() as session:
        if not await delete_user_bun(session, id):
            raise HTTPException(status_code=404, detail="UserBun not found")
        return {"message": f"UserBun {id} deleted"}
...
# --- Эндпоинты для настроек игры (GameSetting) ---
@app.get("/game_settings/", response_model=List[GameSettingModel])
async def get_game_settings_endpoint():
    async with async_session() as session:
...
@app.post("/game_settings/", response_model=GameSettingModel)
async def create_game_setting_endpoint(setting: GameSettingModel):
    async with async_session() as session:
        new_setting = await add_game_setting(
...
@app.put("/game_settings/{key}", response_model=GameSettingModel)
async def update_game_setting_endpoint(key: str, setting: GameSettingModel):
    async with async_session() as session:
        updated_setting = await update_game_setting(
...
@app.delete("/game_settings/{key}")
async def delete_game_setting_endpoint(key: str):
    async with async_session() as session:
        if not await delete_game_setting(session, key):
            raise HTTPException(status_code=404, detail="Setting not found")
        return {"message": f"Setting {key} deleted"}
